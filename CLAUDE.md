...
이 저장소 루트의 `포트폴리오_Unreal_박인호.pdf`와, **비공개로 분리된 private 저장소 `https://github.com/inhopark/Resume`**(공개 저장소에서 개인 문서를 숨기려고 2026-09-23에 이동)에 있는 `자기소개서_박인호.pdf`, `경력기술서_박인호.pdf`는 원본 문서가 아니라 **HTML을 헤드리스 Chrome/Edge로 인쇄해 만든 PDF**입니다(`grep -a -o "Producer\|Creator" 자기소개서_박인호.pdf`로 확인하면 `Producer (Skia/PDF m153)` / `Creator (Mozilla/5.0 ...)`가 나옴 — 즉 python-docx/pandoc 등이 아니라 브라우저 인쇄 파이프라인 산출물). 이 중 하나를 수정해 달라는 요청을 받으면 항상 다음 절차를 따를 것(자기소개서/경력기술서는 이 공개 저장소에 다시 커밋하지 말고 `Resume` 저장소에서 작업할 것):
...
6. **폰트 크기/굵기는 `경력기술서_박인호.pdf`(2026-09 개정판) 기준을 그대로 맞춘다** — 사용자가 이 크기감을 명시적으로 마음에 들어해 표준으로 삼기로 함:
   - 본문 텍스트: 10pt, `line-height: 1.55`
   - 문서 타이틀(`h1`, 예: "경력기술서"): 22pt, `font-weight: 800`
   - 섹션 제목(`h2`, 번호 배지 옆): 13pt / 배지(`.badge`) 숫자: 10pt, `font-weight: 700`
   - 프로젝트명(`.proj-title`, 예: `"The Patcher"`): 10.5pt, `font-weight: 700` — 스튜디오·기간(`.proj-meta`): 8.5pt, 연회색
   - 소제목 초록 필 태그(`.subtag`, 예: "핵심 게임 시스템 개발"): 8.3pt, `font-weight: 700`
   - 역량/성과 옆의 작은 초록 필 태그(`.tag`, 예: "Unreal", "The Patcher"): 7.8pt, `font-weight: 600`
   - 세부 불릿(`ul.bullets li`): 9pt
   이 기준에서 벗어나게 바꾸려면(예: 더 크게/작게) 반드시 사용자에게 먼저 확인할 것.
7. **사용자가 "이 섹션은 몇 쪽부터 시작"처럼 특정 섹션을 특정 페이지로 명시적으로 지정하면**, 5번 항목의 "큰 컨테이너에 break-inside:avoid 금지" 원칙은 그대로 지키되, 해당 섹션/하위 제목 요소에만 `.page-start { break-before: page; }` 클래스를 추가해 강제로 새 페이지를 시작시킨다(자동 리플로우에 맡기지 않음). 여러 지점에 걸 수 있으며(예: 3번 섹션 제목, 4번 섹션 제목, 4번 안의 C# 소제목, 5번 섹션 제목에 각각 적용), 목차(TOC)의 `p.N` 표기가 실제 렌더링 결과와 일치하는지 반드시 다시 렌더링해서 확인한 뒤 필요하면 TOC도 함께 고친다. 특정 구간의 분량이 지정한 페이지 안에 물리적으로 안 들어가면(margin/line-height를 줄여도 안 맞으면) 임의로 더 줄이지 말고 사용자에게 옵션(페이지 늘리기 / 섹션 더 쪼개기 / 해당 구간만 폰트 축소 / 내용 자체를 축약)을 제시해 확인받는다 — 폰트 축소가 선택되면 6번 표준 크기를 문서 전체가 아니라 **그 구간에만** 스코프를 좁힌 클래스(예: `.proj-block.compact`)로 적용하고, 6번 표준 자체는 건드리지 않는다.
8. **동일한 HTML 파일 경로로 여러 번 다시 렌더링했는데 CSS/내용을 바꿨음에도 PDF 결과가 전혀 안 바뀌면, Chrome의 로컬 디스크 캐시를 의심할 것.** 헤드리스 Chrome이 `file://` 리소스를 이전 `--print-to-pdf` 실행 결과로 캐싱해 최신 수정사항을 반영하지 않는 사례가 실제로 있었다. 이럴 땐 매번 새 프로필로 강제 렌더링해서 재확인한다: `--user-data-dir="<매번 다른 임시 경로>" --disk-cache-dir="<매번 다른 임시 경로>"`를 렌더링 명령에 추가(예: 경로 끝에 `$RANDOM`처럼 매번 바뀌는 값을 붙임). 렌더링이 끝나면 이 임시 프로필/캐시 디렉터리는 정리한다.
